...
def test_calculate_droplet_temperature(setup_model):
    ...
    assert (
        temperature_results is not None
    ), "Droplet temperature calculation should return results"
    assert all(
        temperature_results.y[0] >= 0
...
def test_calculate_droplet_position(setup_model):
    ...
    assert len(position) == len(
        time
    ), "Position array should match the length of the time array"
